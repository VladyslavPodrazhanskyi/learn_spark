...
def _retrieve_adjusted_value(row):
    time.sleep(0.0007 * 4)  # limit number of calls per second
...
def write_result(result_df, path):
    ...
    result_df = result_df.repartition("year", "month").cache()
    result_df.printSchema()
...
if __name__ == '__main__':
    ...
    vin_df = source_df.withColumn(
        ...
        & (F.year(F.col("SBMT_date")) == 2020)
        & F.month(F.col("SBMT_date")).isin([9])
    ).dropDuplicates()
    ...
    print(vin_with_book_keys_df.count())

    print('start mapping...')

    adjusted_value_df = Map.apply(
        frame=DynamicFrame.fromDF(vin_with_book_keys_df, glueContext, 'adjusted_value_df'),
        f=_retrieve_adjusted_value
    ).toDF().cache()

    # print(adjusted_value_df.count())
    # adjusted_value_df.printSchema()
    # print("after_mapping_part num", adjusted_value_df.rdd.getNumPartitions())
    ...
    # print("repart_ym_df count", repart_ym_df.counat())
    ...
    # write_result(adjusted_value_df, f"s3://{processed_bucket}/awsdri/pv_tests/adj_value_repart_10")
